...
PayJar is a simple, lightweight interpreter for the **PJ** esoteric programming language, written in C.  The language is designed to be familiar to Java/JavaScript developers with a minimal syntax, modern features and a tiny runtime.
...
You can compile the interpreter with the provided `makefile`:

```sh
# build interpreter binary (pjc) and runtime tool (pjrt)
make all
```

The resulting executables (`pjc` and `pjrt`) appear in the project root.  A pre‑built binary is also included for convenience.

## Running Programs

Compile and run a program with the interpreter:

```sh
./pjc examples/hello.pj      # parse and execute immediately
```

The `pjrt` tool can be used to scan a directory and run packages (see below):

```sh
# run the package in the current directory with optional debug output
./pjrt -d .
```

## Examples

The `examples/` directory contains two sample programs:
...
The `examples/package-ex` directory has a package example.
